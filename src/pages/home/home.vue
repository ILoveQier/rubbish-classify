...
export default {
  ...
  methods: {
    async withDraw() {
      ...
      let amount = 2
      let { data: data1, res } = await this.$wxUtils.request(this.$api.GetPlatFormLoginUrl, this, { amount })
      let { res: res1 } = await this.$wxUtils.request(data1, this)
      this.$wxUtils.showModal({ content: res.errmsg })
      // if (res.errno === 0) {
